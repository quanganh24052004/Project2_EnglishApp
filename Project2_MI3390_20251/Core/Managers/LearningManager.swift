...
class LearningManager {
    ...
    // MARK: - 1. API: Đánh dấu đã học
    // 👇 THAY ĐỔI: Thêm @discardableResult và kiểu trả về StudyRecord?
    @discardableResult
    func markAsLearned(wordID: PersistentIdentifier, supabaseUserID: String?) -> StudyRecord? {
        guard let word = modelContext.model(for: wordID) as? Word else { return nil }
        
        let targetUser: User
        
        // 1. Xác định User (Giữ nguyên logic cũ)
        ...
        var recordToReturn: StudyRecord
        ...
        do {
            ...
            // 👇 THAY ĐỔI: Trả về record để bên ngoài biết ngày nextReview
