...
//  Project2_MI3390_20251
//
//  Refactored: Dashboard & Upcoming Count Logic
//
...
struct ReviewView: View {
    ...
    // 1. Lọc các bản ghi đã đến hạn (NextReview <= Now)
    var dueRecords: [StudyRecord] {
        return studyRecords.filter { $0.nextReview <= currentTime }
    }
    
    // 2. Tìm thời gian review tiếp theo gần nhất (Mốc thời gian tương lai gần nhất)
    var nextReviewDate: Date? {
        return studyRecords
            .filter { $0.nextReview > currentTime }
            .map { $0.nextReview }
    ...
    // 3. [NEW] Tính tổng số từ chuẩn bị ôn tập (trong đợt sắp tới)
    // Logic: Đếm số từ có lịch review trong vòng 60 phút kể từ mốc nextReviewDate
    var upcomingCount: Int {
        guard let nextDate = nextReviewDate else { return 0 }
        
        // Tạo cửa sổ thời gian: Mốc gần nhất + 60 phút
                ...
                VStack(spacing: 24) {
                    ...
                    VStack {
                        if !dueRecords.isEmpty {
                            ...
                            // CASE B: Chưa có bài -> Hiện Countdown + Thông tin sắp tới
                            if let nextDate = nextReviewDate {
                                VStack(spacing: 12) {
                                    Text("Bạn đã hoàn thành tất cả bài ôn tập! 🎉")
                                        .font(.subheadline)
                                    ...
                                    // [NEW] Hiển thị số lượng từ sắp tới
                                    HStack(spacing: 4) {
                                        Text("Sắp có")
                                            .foregroundColor(.gray)
                                        Text("\(upcomingCount)")
